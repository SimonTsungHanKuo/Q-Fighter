...
import numpy as np
import qtable_io
from qtable_io import QTableIO
...
#                self.moveVect = Vector(0,0)
    ...
    def get_hit_light(self, anim = -1):
        ...
        # -----Q-Fighter by Vitor, Henry:2019-06-09-----
        self.moveVect = self.vector_facing(Vector(-1.0, 0))
...
class Game:
    ...
    #-----Q-Fighter by Simon, Henry:2019-06-11-----
    learning_mode = True
    #learning_mode = False
    episode = 0
    episode_threshold = 3200
    #episode_threshold = 0
    action_comblow = 0  
    action_comblow_step = 0    
    last_m_health = 0   

    n_buckets = (4, 5)        # Observation space: [distance: 0-25, 26-50, 50-75, 76-100, > 100; health loss: 0, <25, <50, <75, <100]
    n_actions = 18            # Action Space : [combination blow = 0-16, back only = 16, forward only = 7]

    ## init Q table for each state-action pair
    q_table_data_file_saved = False
    q_table_data_file = 'qtable_data'
    qt = QTableIO()
    if os.path.isfile(q_table_data_file + ".txt"):
       #-----Q-Fighter by Simon, Vitor:2019-06-12-----
       print ("Q-Table loaded from file: ", q_table_data_file+".txt")
       q_table = qt.q_table_load(q_table_data_file, n_buckets+(n_actions,))
       q_table_data_file_saved = False
    else: 
    ...
    def __init__(self, screen, background, character1, character2):
        ...
        #-----Q-Fighter by Simon:2019-05-27-----
        self.env = gym.make('gym_qfighter:qfighter-v0')
        self.state = self.get_state()
        self.last_m_health = self.character1.health.health
        if Game.learning_mode == False:
           Game.episode_threshold = 0
        #---------------------------------------
...
#        surface.convert_alpha()
    ...
    #-----Q-Fighter by Simon, Yu-Yen:2019-06-11-----
    def choose_action(self, state, q_table, action_space):
        """
        action : move [0=back or 1=forw or 2=down or 3= up] + attack [A, B, C]
        
        action  move       attack     desc.
        =================================
        (Back Part)
        ---------------------------------
        0       0          A          (Back + A)
        ...
        9       1          B          (Forw + B)
        10      1          C          (Forw + C)
        11      1+2        A          (Forw + Down + A)
        12      1+2        B          (Forw + Down + B)
        13      1          A+B        (Forw + A    + B)
        14      2+1        A          (Down + Forw + A)
        15      1+2+1      B          (Forw + Down + Forw + B)
      
        (Go / Back Straight Part) 
        ---------------------------------
        16      0          None       (Back Only)
        17      1          None       (Forw Only)
        ...
        InputReader.move += 1
        if InputReader.move % 100 < 50:
           action = 16 # back only
        else:
           action = 17 # forward only
        
        ## state[0] (distance:[0: < 30, 1: < 60, 2: < 150, 3: others ])
        if state[0] == 3: # Attacking while distance is less than 90 
           print ("Attack mode: None ", "Attack Command: ", action)
           return action

        else: # Nearly 
          #-----Q-Fighter by Simon, Yu-Yen:2019-06-11-----
          if Game.episode < Game.episode_threshold: # current config is 1600
             if Game.action_comblow == 0:
                action = action_space.sample()  
                if action == 3 or action == 4 or action == 5 or action == 6 or action == 7 \
                   or action == 11 or action == 12 or action == 13 or action == 14 or action == 15:
                   Game.action_comblow = action
                   Game.action_comblow_step = 0
             else:
                action = Game.action_comblow
            
             print ("Attack mode: Random Training =", action)
             ...
             print ("Attack mode: Q-Table Greedy = ", action)

             #if attack == 2 and self.character2.energy.energy < 25:
             #   attack = 0
             #   print ("Attack mode: Low energy light attack = ", attack)
             # print ("Attack mode: Q-Table Greedy = ", attack)

          return action

    def get_state(self):
        # evaluate distance
        ## state[0] (distance:[0: < 15, 1: < 30, 2: < 45, 3: others ])
        ## state[0] (distance:[0: < 30, 1: < 60, 2: < 90, 3: others ])
        ## state[0] (distance:[0: < 30, 1: < 60, 2: < 150, 3: others ])
        dist = abs(self.character1.position.x - self.character2.position.x)
        if dist < 30:
           state_dist = 0
        ...
        # evaluate opponent health loss
        health_loss = self.last_m_health - self.character1.health.health
        self.last_m_health = self.character1.health.health
        if health_loss == 0:
           state_health_loss = 0
        elif health_loss < 25:
           ...
           state_health_loss = 3 
        elif health_loss < 100:
           state_health_loss = 4 
        else: 
           state_health_loss = 0

        # evaluate CPU Player energy
        # print ("CPU Energy", self.character2.energy.energy)

        return tuple((state_dist, state_health_loss))
    #---------------------------------------
    ...
    def mainloop(self):
        ...
        while True:
            ...
            Game.episode += 1
            lr = get_lr(Game.episode)
            """

            state = self.get_state()
            print ("==================  State Desc. =======================")
            print ("state       [state[0]: distance, state[1]: health loss]")
            print ("distance    [0: < 15, 1: < 30, 2: < 45, 3: others ]")
            print ("health loss [0: False, 1: True]")
            print ("move        [0: Back,  1: Forw]")
            print ("------------------ Current State  ---------------------")
            print ("State[0]:    ", state[0], "  State[1]:   ", state[1])
            print ("=======================================================")

            c_action = self.choose_action(state, self.q_table, self.env.action_space)
            stick_inputs_p1, btn_inputs_P1, stick_inputs_p2, btn_inputs_P2, special = \
                                                self.input_reader.getInputs(c_action, self.character1.facingRight)
            #---------------------------------------
    ...
    #                self.character1.print_inputs()
            ...
            """ 
                  move to bottom  by Simon 06121800 
            """
            ...
            if end_round and self.winpose():
                ...
                #-----Q-Fighter by Simon:2019-06-11-----
                if Game.episode < Game.episode_threshold:
                   # """
                   #-----Q-Fighter by Simon:2019-06-11-----
                   # Learning related constants; factors determined by trial-and-error
                   # epsilon-greedy, factor to explore randomly; discounted over time
                   ...
                   reward = self.env.step(next_state, c_action)

                   # Agent learns via Q-learning
                   q_next_max = np.amax(self.q_table[next_state])
                   self.q_table[state + (c_action,)] += lr * (reward + gamma * q_next_max - self.q_table[state + (c_action,)])
                   ...
                   print ("q_table: ")
                   print (self.q_table)
                   print ("###############  End-of-Episode Q-Value Updated  ###############")
                   print ("                                                 ")
                   print ("                                                 ")
                else:
                   if not Game.q_table_data_file_saved:
                      print ("Q-Table Saved !")
                      Game.qt.q_table_save (self.q_table, Game.q_table_data_file)
                      Game.q_table_data_file_saved = True
...
#                else: self.clock.tick(15)
            ...
            if Game.episode < Game.episode_threshold:
               ...
               q_next_max = np.amax(self.q_table[next_state])
               self.q_table[state + (c_action,)] += lr * (reward + gamma * q_next_max - self.q_table[state + (c_action,)])
               #self.q_table[state + (c_action,)] = round (self.q_table[state + (c_action,)], 5)
               ...
               print ("###############  Q-Learning is Completely  ###############")
               print ("                                                 ")
               print ("                                                 ")
...
class InputReader:
    ...
    #-----Q-Fighter by Simon:2019-06-12-----
    move = 0
    cpu_player = True
    # cpu_player = False
    cpu_player_action_list = []
    cpu_player_move = 0
    cpu_player_action = 0
    #---------------------------------------
    ...
    def getInputs(self, c_action, player1_facingRight):  # Q-fighter, funct. inputs modified by Vitor, 2019-06-13
    #def getInputs(self, c_action):
    #def getInputs(self):
        btn_inputs_p1 = []
        stick_inputs_p1 = []
        btn_inputs_p2 = []
        ...
        #-----Q-Fighter by Simon:2019-05-27-----
        env = gym.make('gym_qfighter:qfighter-v0')

        #-----Q-Fighter by Simon, Vitor:2019-05-27-----

        # -----Q-Fighter modified by Vitor:2019-06-12-----

        # waiting time for round start animation:
        if not InputReader.cpu_player_action_list:
            for i in range(9):
                InputReader.cpu_player_action_list.append([KEYCONST.NULL, KEYCONST.NULL])

            #### P1 cpu self-defined playing strategies:####
            # reminder: 1 frame = 3 mainloop, that means if you want to input nothing for 1 frame,
            # you should append KETCONST.NULL 3 times.
            ## ----Fireball-denfence strategy:----
            if player1_facingRight:
                # fireball inputs of cpu player is facing right:
                InputReader.cpu_player_action_list.append([KEYCONST.DOWN, KEYCONST.NULL])
                InputReader.cpu_player_action_list.append([KEYCONST.FORW, KEYCONST.BTNA])
                InputReader.cpu_player_action_list.append([KEYCONST.DOWN, KEYCONST.NULL])
                InputReader.cpu_player_action_list.append([KEYCONST.FORW, KEYCONST.BTNA])

                # keep moving back to defend any attacks from the enemy for 12 frames.
                for i in range(36):
                    InputReader.cpu_player_action_list.append([KEYCONST.BACK, KEYCONST.NULL])
            else:
                # fireball inputs of cpu player is facing left:
                InputReader.cpu_player_action_list.append([KEYCONST.DOWN, KEYCONST.NULL])
                InputReader.cpu_player_action_list.append([KEYCONST.BACK, KEYCONST.BTNA])
                InputReader.cpu_player_action_list.append([KEYCONST.DOWN, KEYCONST.NULL])
                InputReader.cpu_player_action_list.append([KEYCONST.BACK, KEYCONST.BTNA])

                # keep moving back to defend any attacks from the enemy for 12 frames.
                for i in range(36):
                    InputReader.cpu_player_action_list.append([KEYCONST.FORW, KEYCONST.NULL])

            ##----Fireball-defence strategy end----

            # reverse the whole action list for poping up actions from the first appended element.
            InputReader.cpu_player_action_list.reverse()

            InputReader.cpu_player_move, InputReader.cpu_player_action = InputReader.cpu_player_action_list.pop()
        else:
            InputReader.cpu_player_move, InputReader.cpu_player_action = InputReader.cpu_player_action_list.pop()
            # ----------------------------------------------
        ...
        for event in pygame.event.get():
            ...
            if event.type == KEYDOWN:
                ...
                #-----Q-Fighter by Simon, Vitor:2019-06-09-----
                if InputReader.cpu_player == False:
        ...
        if InputReader.cpu_player == False:
           ...
           #-----Q-Fighter by Simon, Vitor:2019-06-09-----
           if InputReader.cpu_player_move != -1:
              stick_inputs_p1.append(InputReader.cpu_player_move)   
           if InputReader.cpu_player_action != -1:
              btn_inputs_p1.append(InputReader.cpu_player_action) 
           #-----Q-Fighter by Simon, Vitor:2019-06-09-----
        ...
        if  c_action <= 7:
            stick_inputs_p2.append(KEYCONST.BACK)
            if c_action == 0:				# (Forw + A)
                btn_inputs_p2.append(KEYCONST.BTNA)
            elif c_action == 1:                         # (Forw + B)
                btn_inputs_p2.append(KEYCONST.BTNB)
            elif c_action == 2:                         # (Forw + C)
                btn_inputs_p2.append(KEYCONST.BTNC)
            elif c_action == 3:                         # (Back + Down + A)
                Game.action_comblow_step += 1
                if Game.action_comblow_step == 2:
                   stick_inputs_p2.pop()
                   stick_inputs_p2.append(KEYCONST.DOWN)
                elif Game.action_comblow_step == 3:
                   btn_inputs_p2.append(KEYCONST.BTNA)
                   Game.action_comblow = 0 
            elif c_action == 4:                         # (Back + Down + B)
                Game.action_comblow_step += 1
                if Game.action_comblow_step == 2:
                   stick_inputs_p2.pop()
                   stick_inputs_p2.append(KEYCONST.DOWN)
               ...
               if Game.action_comblow_step == 2:
                  ...
                  btn_inputs_p2.append(KEYCONST.BTNB)
                  Game.action_comblow = 0   
            elif c_action == 6:                         # (Down + Back  + A)
                Game.action_comblow_step += 1
                if Game.action_comblow_step == 1:
                    stick_inputs_p2.pop()
                    stick_inputs_p2.append(KEYCONST.DOWN)
                elif Game.action_comblow_step == 3:
                    btn_inputs_p2.append(KEYCONST.BTNA)
                    Game.action_comblow = 0   
            elif c_action == 7:                         # (Back + Down + Back + B)
                Game.action_comblow_step += 1
                if Game.action_comblow_step == 2:
                    stick_inputs_p2.pop()
                    stick_inputs_p2.append(KEYCONST.DOWN)
                elif Game.action_comblow_step == 4:
                    btn_inputs_p2.append(KEYCONST.BTNB)
                    Game.action_comblow = 0

        if c_action < 16 and c_action > 8: 
            stick_inputs_p2.append(KEYCONST.FORW)	
            if c_action ==   8:                         # (Forw + A)
               btn_inputs_p2.append(KEYCONST.BTNA)      
            elif c_action == 9:                         # (Forw + B)
               btn_inputs_p2.append(KEYCONST.BTNB)      
            elif c_action == 10:                        # (Forw + C)
               btn_inputs_p2.append(KEYCONST.BTNC)
            elif c_action == 11:			# (Forw + Down + A)
               Game.action_comblow_step += 1
               if Game.action_comblow_step == 2:
                  stick_inputs_p2.pop()
                  stick_inputs_p2.append(KEYCONST.DOWN)
               ...
               if Game.action_comblow_step == 2:
                ...
                if Game.action_comblow_step == 1:
                    ...
                    btn_inputs_p2.append(KEYCONST.BTNA)
                    Game.action_comblow = 0   
            elif c_action == 15:                        # (Forw + Down + Forw + B)
                Game.action_comblow_step += 1
                if Game.action_comblow_step == 2:
                    stick_inputs_p2.pop()
                    stick_inputs_p2.append(KEYCONST.DOWN)
